...
class AbsPrintLoggerTest {
    ...
    fun testPrintMessage() {
        ...
        logger.doLog(LogLevel.DEBUG, "Tag", "Legs are tied, these hands are broken")
        assertEquals("D/Tag: Legs are tied, these hands are broken", logger.logMessage)

        logger.doLog(LogLevel.INFO, "Another Tag", "Alone I try with words unspoken")
        assertEquals("I/Another Tag: Alone I try with words unspoken", logger.logMessage)

        logger.doLog(LogLevel.WARNING, "OK", "Silent cry, my breath is frozen")
        assertEquals("W/OK: Silent cry, my breath is frozen", logger.logMessage)

        logger.doLog(LogLevel.ERROR, null, "With blinded eyes, I fear myself!")
    ...
    fun testPrintException() {
        ...
        logger.doLog(LogLevel.DEBUG, "TEST", RuntimeException("It's burning down, it's burning high"))
        assertEquals("D/TEST: <Exception Name>: It's burning down, it's burning high\n" +
                "<Exception StackTrace>", logger.logMessage)

        logger.doLog(LogLevel.INFO, null, RuntimeException("When ashes fall the legends rise"))
        assertEquals("I/: <Exception Name>: When ashes fall the legends rise\n" +
                "<Exception StackTrace>", logger.logMessage)

        logger.doLog(LogLevel.WARNING, "We burn it out a mile wide", RuntimeException())
        assertEquals("W/We burn it out a mile wide: <Exception Name>\n" +
                "<Exception StackTrace>", logger.logMessage)

        logger.doLog(LogLevel.ERROR, null, RuntimeException("When ashes fall, the legends rise"))
